...
module.exports = {
  name: 'server',
  description: 'Show bot server location info',
  async execute(sock, msg, args) {
    let tempImgPath = null;
    let browser = null;

    try {
      await sock.sendMessage(msg.key.remoteJid, { text: '⏳ Loading...' }, { quoted: msg });
      ...
      // 5. Kirim Teks dengan externalAdReply Gambar Besar
      const imageBuffer = fs.readFileSync(tempImgPath);
      const caption = `🌐 *- S E R V E R -*\n\n${textDataset}\n\n🔗 *Source* : ${targetUrl}`;
      ...
      await sock.sendMessage(
        ...
        { 
          ...
          contextInfo: {
            externalAdReply: {
              title: "SERVER LOCATION INFO",
              body: `IP Address: ${myIp}`,
              mediaType: 1,
...
// [berhasil] fitur untuk mengetahui server bot ✓
