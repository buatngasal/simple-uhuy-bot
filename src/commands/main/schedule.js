...
module.exports = {
  name: 'schedule',
  description: `Schedule messages: ${commandPrefix}schedule <HH:MM> <message>, ${commandPrefix}schedule list, ${commandPrefix}schedule remove <id>`,
  usage: `${commandPrefix}schedule <HH:MM> <message> | ${commandPrefix}schedule list | ${commandPrefix}schedule remove <id>`,
  async execute(sock, msg, args) {
    try {
      ...
      if (cmd === 'remove') {
        ...
        if (isNaN(idx) || idx < 0) return sock.sendMessage(id, { text: `Usage: ${commandPrefix}schedule remove <id> (id must be a positive number)` }, { quoted: msg });
      ...
      if (!/^\d{2}:\d{2}$/.test(time) || !message) {
        return sock.sendMessage(id, { text: `Usage: ${commandPrefix}schedule <HH:MM> <message>` }, { quoted: msg });
      }
